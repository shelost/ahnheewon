...
function newBlock(e){
    ...
    CURSOR_SIZE = CURSOR_SIZE * 4 / 3


    setTimeout(() => {
        CURSOR_SIZE = CURSOR_SIZE * 3/4
    }, 200);

    /*
    CURSOR.x =
    CURSOR.w = CURSOR_SIZE * 1.25
    CURSOR.h = CURSOR_SIZE * 1.25
    CURSOR.r = CURSOR_SIZE * 1.25 / 2

    setTimeout(() => {
        CURSOR.w = CURSOR_SIZE
        CURSOR.h = CURSOR_SIZE
        CURSOR.r = CURSOR_SIZE/2
    }, 200);
    */
}
...
let HOVER_ADD = 0
let HOVER_UP = true
const HOVER = new Squircle(HX, HY, HOVER_SIZE, HOVER_SIZE, HOVER_SIZE)
const CURSOR = new Squircle(HX, HY, CURSOR_SIZE, CURSOR_SIZE, CURSOR_SIZE/2)

// Buttons
var Button = new Squircle(canvas.width-GR_X*2.4, canvas.height-GR_X*2.4, GR_X*2,GR_X*2,GR_X)
var startButton = new Squircle(CENT_X-GR_X*2, CENT_Y+GR_X*2, GR_X*3.5, GR_X*1.5, GR_X*1)
...
const loop = ()=>{
    ...
    if (con.up && !player.jumping || con.space && !player.jumping){
    ...
    for (u=0;u<blocks.length;u++){
       ...
       if (level != 18){
           //let supp = new Squircle(block.x-2*ONE, block.y-2*ONE, block.w+4*ONE, block.h+4*ONE, block.r*2)
           switch (phase){
                case 1:
    ...
    for(r=0;r<player.history.length; r++){
        ...
        ctx.beginPath()
        ctx.arc(player.history[r][0], player.history[r][1], player.w/3.5, 0, Math.PI*2)
        ctx.fill()
        ctx.globalAlpha = 1
    }
    ...
    ctx.fillStyle = 'white'
    ctx.beginPath()
    ctx.ellipse(player.x + (player.right ? player.w * 0.55 : player.w * 0.45), player.y + player.yadd, player.w / 10, player.h / 5, 0, 0, Math.PI * 2, false)
    ctx.ellipse(player.x + (player.right ? player.w * 0.82 : player.w * 0.18), player.y + player.yadd, player.w / 15, player.h / 5.2, 0, 0, Math.PI * 2, false)
   // ctx.ellipse(player.x + player.w- player.xadd, player.y + player.yadd, player.w/10, player.h/5, 0, 0, Math.PI*2, false)
    ctx.closePath()
    ctx.fill()
    ...
    if (scene == 3){
        ...
        ctx.font = `bold ${ONE*16}px Inter`

        ctx.fillText(`Level ${level}`, 20*ONE,40*ONE)

        ctx.font = `bold ${ONE * 14}px Inter`
            let txt = `${limit - blocks.length + 2}`
            let tw = ctx.measureText(txt)
            ctx.fillStyle = 'white'
            ctx.globalAlpha = 0.7
            let b = new Squircle(canvas.width - 125 * ONE, 20 * ONE, 130 * ONE, 29*ONE, 20)
            b.draw()
            ctx.globalAlpha = 1
            ctx.fillStyle = 'black'
            ctx.fillText(limit - blocks.length + 2, canvas.width - 115 * ONE, 40 * ONE)
            ctx.font = `${ONE * 13}px Inter`
            ctx.fillText('Blocks Left', canvas.width - 110 * ONE + tw.width, 40 * ONE)

        /*
        if (limit-blocks.length+2 ==1){
            ctx.fillStyle = 'white'
            ctx.globalAlpha = 0.7
            let b = new Squircle(canvas.width - 125 * ONE, 20 * ONE, 100*ONE, 29*ONE, 20)
            b.draw()
            ctx.globalAlpha = 1
            ctx.fillStyle = 'black'
            ctx.fillText(`1  Block Left`, canvas.width - 115 * ONE, 40 * ONE)
        } else {
        }
        */


        ctx.font = `${ONE*14}px Inter`
        ctx.globalAlpha = 0.5
        ctx.fillText(caption, 20*ONE, 64*ONE)
        ctx.globalAlpha = 1
    }
    ...
    if (scene == 1){
        ...
        ctx.drawImage(logo, CENT_X - GR_X*6, CENT_Y - GR_X*2.5, GR_X*12 , GR_X*3)
        ctx.shadowColor = 'transparent'
        ...
        startButton.draw()
        ctx.drawImage(title, startButton.x+GR_X*0.22, startButton.y, startButton.w*0.85, startButton.h)
    }
    ...
    if (scene == 2){
        ...
        for (i=0;i<grid.length;i++){
            ...
            for (j=0;j<grid[0].length;j++){
                ...
                ctx.fillStyle = 'white'
                ctx.font = `bold ${GR_X*0.6}px Inter`
                if (grid[i][j] < 10){
    ...
    /*
    ctx.globalAlpha = 0.2
    let b = new Squircle(HX, HY, 70, 70, 30)
    b.draw()
    ctx.globalAlpha = 1
    let c = new Squircle(HX, HY, 30, 30, 10)
    c.draw()
    */
    ...
    */
    ...
    let lim = limit - blocks.length + 2

    if (lim > 0) {
        let HOVER_FINAL = HOVER_SIZE + HOVER_ADD
        ctx.globalAlpha = 0.1
        HOVER.w = HOVER_FINAL
        HOVER.h = HOVER_FINAL
        HOVER.x = HX - HOVER_FINAL/2
        HOVER.y = HY - HOVER_FINAL / 2
        HOVER.draw()
    }

    ctx.globalAlpha = 1
    CURSOR.x = HX - CURSOR_SIZE/2
    CURSOR.y = HY - CURSOR_SIZE / 2
    CURSOR.w = CURSOR_SIZE
    CURSOR.h = CURSOR_SIZE
    CURSOR.r = CURSOR_SIZE/2
    CURSOR.draw()

    ctx.font = `bold ${8 * ONE}px Inter`
    ctx.fillStyle = 'white'
    ctx.fillText(lim, HX-ctx.measureText(lim).width/2, HY+3*ONE)
